...
export async function ensureCodeWorkspaceContainer(id) {
  ...
  const { skillsDir } = resolveAgentScope(repoDir, workspace.scope || null);
  const systemPrompt = buildCodingAgentSystemPrompt(isAgent ? 'agent' : 'code', isAgent ? skillsDir : null);
...
export async function startInteractiveMode(id, agentOverride) {
  ...
  const { skillsDir: scopedSkillsDir } = resolveAgentScope(repoPath, workspace.scope || null);
  const systemPrompt = buildCodingAgentSystemPrompt(isAgent ? 'agent' : 'code', isAgent ? scopedSkillsDir : null);
...
export async function launchWorkspaceCommand(id, command) {
  ...
  try {
    const { workspaceDirExists, runCommandContainer } =
      await import('../tools/docker.js');
    ...
    console.error(`[launchWorkspaceCommand] workspace=${id} command=${command}`, err);
    return { success: false, message: err.message };
  }
}
